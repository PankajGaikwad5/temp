...
function fitToView(obj, cam) {
  ...
  // Fixed near — prevents clipping when OrbitControls zooms in
  cam.near = 0.01
  cam.far  = z * 20
...
function loadGLB(index, s, onProgress) {
  ...
  return new Promise((resolve, reject) => {
    ...
    loader.load(
      CATS[index].file,
      gltf => { setShadows(gltf.scene); s.models[key] = gltf.scene; resolve(gltf.scene) },
...
function useThreeScene(containerRef, onProgress, onLoaded) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // HDR environment — handles all lighting naturally
      ...
      glowRing.rotation.x = -Math.PI / 2; glowRing.position.y = -2.19
      scene.add(glowRing); s.glowRing = glowRing
      ...
      s.isInteracting = () => userInteracting
      ...
      const loop = () => {
        ...
        s.floatTime += dt
        if (s.activeGroup && !s.isInteracting()) {
      ...
      } catch (e) { console.warn('GLB load failed — check /public/ for .glb files:', e) }

      if (!cancelled) {
        clearTimeout(safety); onLoaded()
...
// ─── STYLES (vw-responsive) ───────────────────────────────────────────────────
const S = {
  navPad:      { padding: '0 clamp(36px, 4vw, 110px)' },
  wordmark:    { fontSize: 'clamp(12px, 1vw, 18px)' },
  wordmarkSub: { fontSize: 'clamp(9px, 0.65vw, 13px)' },
  navLink:     { fontSize: 'clamp(10px, 0.75vw, 13px)' },
  sideMargin:  { left:  'clamp(36px, 4vw, 110px)' },
  sideMarginR: { right: 'clamp(36px, 4vw, 110px)' },
  eyebrow:     { fontSize: 'clamp(9px, 0.75vw, 13px)' },
  desc:        { fontSize: 'clamp(13px, 1.1vw, 19px)', maxWidth: 'clamp(300px, 26vw, 520px)', lineHeight: 1.75 },
  cta:         { fontSize: 'clamp(10px, 0.75vw, 13px)', padding: 'clamp(12px,1vw,18px) clamp(26px,2vw,44px)' },
  counter:     { fontSize: 'clamp(52px, 5.5vw, 110px)' },
  counterSub:  { fontSize: 'clamp(12px, 1.1vw, 20px)' },
  counterLabel:{ fontSize: 'clamp(8px, 0.6vw, 12px)' },
  tabLabel:    { fontSize: 'clamp(9px, 0.7vw, 12px)' },
  scrollLabel: { fontSize: 'clamp(8px, 0.6vw, 11px)' },
}
...
export default function HeroPage() {
  ...
  const switchCat = useCallback((index) => {
    ...
    setTextVisible(false); setCatIdx(index); switchTo(index)
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-[200] bg-ivory flex flex-col items-center justify-center gap-6 transition-[opacity,visibility] duration-700 ${loaderDone ? 'opacity-0 invisible' : ''}`}>
        <span className="text-crimson tracking-[0.5em] uppercase font-semibold" style={S.wordmark}>The Vault</span>
        <span className="font-serif italic text-gold-dark -mt-3" style={S.wordmarkSub}>by Karan Desai</span>
        ...
        <span className="text-ink-soft tracking-[0.2em] -mt-3" style={{ fontSize: 'clamp(9px,0.7vw,12px)' }}>{Math.round(progress)}%</span>
      </div>

      {/* ── Nav ── */}
      <nav className="fixed top-0 inset-x-0 z-50 flex items-center justify-between h-[72px] bg-ivory/70 backdrop-blur-lg border-b border-gold/20" style={S.navPad}>
        <ul className="flex gap-8 list-none">
          {CATS.map((c, i) => (
            <li key={c.key}>
              <button onClick={() => switchCat(i)} style={S.navLink} className={`relative group tracking-[0.16em] uppercase font-medium transition-colors ${i === catIdx ? 'text-gold-dark' : 'text-ink-mid hover:text-ink'}`}>
                {c.overlay}
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center pointer-events-none">
          <span className="text-crimson tracking-[0.38em] uppercase font-semibold leading-tight" style={S.wordmark}>The&nbsp;&nbsp;Vault</span>
          <span className="font-serif italic text-ink-soft tracking-[0.2em] mt-0.5" style={S.wordmarkSub}>by Karan Desai</span>
        </div>

        <div className="flex items-center gap-7">
          {['Collection', 'About', 'Contact'].map(l => (
            <a key={l} href="#" style={S.navLink} className="tracking-[0.16em] uppercase font-medium text-ink-mid hover:text-ink transition-colors">{l}</a>
          ))}
          <a href="#" style={{ fontSize: 'clamp(10px,0.75vw,13px)', padding: 'clamp(8px,0.7vw,12px) clamp(16px,1.4vw,26px)' }}
            className="tracking-[0.18em] uppercase font-semibold text-ivory bg-ink hover:bg-gold-dark transition-colors">Shop Now</a>
        </div>
      </nav>
      ...
      <section className="relative w-screen h-screen min-h-[700px] overflow-hidden">
        ...
        <div className="absolute bottom-[5vh] z-10 flex flex-col items-end gap-1 opacity-[0.45] pointer-events-none" style={S.sideMarginR}>
          <span className="font-extralight text-ink leading-none tracking-[-0.04em]" style={S.counter}>
            {cat.counter}<span className="font-normal text-ink-soft align-super" style={S.counterSub}>/03</span>
          </span>
          <div className="w-5 h-px bg-gold" />
          <span className="tracking-[0.3em] uppercase text-ink-soft font-medium" style={S.counterLabel}>Collection</span>
        </div>

        {/* Category tabs — right center */}
        <div className="absolute top-1/2 -translate-y-1/2 z-10 flex flex-col gap-1" style={S.sideMarginR}>
          {CATS.map((c, i) => (
            <button key={c.key} onClick={() => switchCat(i)} className="flex items-center flex-row-reverse gap-3 py-2.5 group">
              <span style={S.tabLabel} className={`tracking-[0.22em] uppercase font-medium whitespace-nowrap transition-colors duration-300 ${i === catIdx ? 'text-gold-dark' : 'text-ink-soft group-hover:text-ink'}`}>
                {c.overlay}
              </span>
              <span className={`h-px transition-all duration-300 ${i === catIdx ? 'w-[52px] bg-gold' : 'w-7 bg-ink-soft group-hover:w-9'}`} />
              <span style={S.counterLabel} className={`text-gold tracking-[0.1em] transition-all duration-300 ${i === catIdx ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-1'}`}>
                0{i+1}
              </span>
            </button>
          ))}
        ...
        <div className={`absolute bottom-[5vh] z-10 hero-text ${textVisible ? 'visible' : ''}`} style={S.sideMargin}>
          <p className="flex items-center gap-3 tracking-[0.3em] uppercase text-gold font-medium mb-[1.2vw]" style={S.eyebrow}>
            <span className="block w-6 h-px bg-gold flex-shrink-0" />
            {cat.eyebrow}
          </p>
          <h1 className="font-light text-ink mb-[1vw] leading-[0.97] tracking-[-0.02em]" style={{ fontSize: 'clamp(42px, 5.5vw, 110px)' }}>
            {cat.title[0]}
            <em className="font-serif font-light text-gold-dark text-[1.18em] tracking-[0.01em] block" style={{ fontStyle: 'italic' }}>
              {cat.title[1]}
            </em>
          </h1>
          <p className="text-ink-soft mb-[2vw]" style={S.desc}>{cat.desc}</p>
          <div className="flex items-center gap-7">
            <a href="#" style={S.cta} className="tracking-[0.2em] uppercase font-semibold text-ivory bg-ink hover:bg-gold-dark transition-colors">{cat.cta}</a>
            <a href="#" style={{ fontSize: 'clamp(10px,0.75vw,13px)' }} className="flex items-center gap-2.5 tracking-[0.2em] uppercase font-medium text-ink-mid hover:text-gold-dark transition-colors group">
              Explore
        ...
        <div className="absolute bottom-9 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2 opacity-40">
          ...
          <span className="tracking-[0.32em] uppercase text-ink-soft font-medium" style={S.scrollLabel}>Scroll</span>
        </div>
      </section>
    </>
  )
